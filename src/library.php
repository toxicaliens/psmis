...
function checkLogin($username, $password) {
    ...
    $psd = $password;
    $result = run_query("SELECT u.*, r.*, m.image_path FROM " . DATABASE . ".$tablename u
        LEFT JOIN user_roles r ON r.role_id = u.user_role
        LEFT JOIN masterfile m ON m.mf_id = u.mf_id
    ...
    if ($num > 0) {
        ...
        $_SESSION['sys_name'] = 'SMIS';
        $_SESSION['school_id'] = $array['school_id'];
        ...
        $_SESSION['prof_pic'] = $array['image_path'];
...
function getDbMenu($parent_id, $active_view, $view_ids){
    ...
    if(get_num_rows($query) > 0){
        echo (is_null($parent_id)) ? '<ul class="nav side-menu">' : '<ul class="nav child_menu">';
...
function checkForExistingEntry2($tablename, $column_name, $id){
  $check_query = "SELECT $column_name FROM $tablename WHERE $column_name = '".$id."' AND school_id = '".$_SESSION['school_id']."'";
  // var_dump($check_query);exit;
  $result = run_query($check_query);
  $num_rows = get_num_rows($result);
  if($num_rows >= 1){
    ...
    while($row = get_row_data($result)){
        ...
        if(is_null($parent_id))
        {
            ...
            echo '<td>'.$menu_name.'</td>';
                ...
                echo "<td>
                    <select name=\"parent_id$menu_id\" class=\"form-control select2_single\" style=\"width: 100%;\">
